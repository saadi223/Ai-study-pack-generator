...
from utils import StudyError, request_json
...
def new_state(inputs):
    return {"fingerprint": fingerprint(inputs), "outputs": {}, "failed_stage": None, "error": None}
...
def stage_call(client, inputs, outputs, name, schema, instruction, notify):
    ...
    return request_json(client, inputs["model"], SYSTEM + "\n" + instruction,
                        ...
                        schema.model_json_schema(), validate, notify)
