...
from utils import (StudyError, RateLimitPause, request_json, wait_with_progress,
                   REQUEST_PACING_SECONDS, MAX_AUTO_WAIT_SECONDS)
...
def new_state(inputs):
    return {"fingerprint": fingerprint(inputs), "outputs": {}, "failed_stage": None,
            "error": None, "retry_at": 0, "next_request_at": 0}
...
def run_workflow(client, inputs, state, progress=None):
    ...
    if state.get("fingerprint") != fingerprint(inputs):
        # Preserve quota pacing when the learner changes inputs in a live session.
        next_request_at = state.get("next_request_at", 0)
        next_request_at = max(next_request_at, state.get("retry_at", 0))
        state.clear()
    ...
    if remaining > 0:
        if remaining > MAX_AUTO_WAIT_SECONDS:
            state["error"] = f"Groq requires a longer cooldown ({remaining} seconds remaining). Completed stages are saved. Check your account Limits before retrying."
            return state
        stage = state.get("failed_stage") or next((s for s in STAGES if s not in state["outputs"]), STAGES[-1])
        notify = (lambda message: progress(len(state["outputs"]), stage, message)) if progress else None
        wait_with_progress(remaining, notify, "Resuming after the saved Groq cooldown")
    state["retry_at"] = 0
    ...
    for index, (name, function) in enumerate(zip(STAGES, FUNCTIONS)):
        ...
        try:
            ...
            remaining = max(0, math.ceil(state.get("next_request_at", 0) - time.time()))
            if remaining > MAX_AUTO_WAIT_SECONDS:
                raise RateLimitPause(f"Groq cooldown still has {remaining} seconds remaining. Check your account Limits before retrying.", remaining)
            if remaining:
                wait_with_progress(remaining, notify, "Spacing AI stages to reduce rate limits")
            state["next_request_at"] = 0
            state["outputs"][name] = function(client, inputs, dict(state["outputs"]), notify)
            state["next_request_at"] = time.time() + REQUEST_PACING_SECONDS
