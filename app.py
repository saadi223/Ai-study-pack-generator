...
def clear_all():
    old = st.session_state.get("workflow", {})
    st.session_state["quota_not_before"] = max(
        old.get("next_request_at", 0), old.get("retry_at", 0),
        st.session_state.get("quota_not_before", 0),
    )
...
def reset_workflow(inputs):
    """New content does not reset the provider's rate-limit clock."""
    old = st.session_state.get("workflow", {})
    fresh = new_state(inputs)
    fresh["next_request_at"] = max(
        old.get("next_request_at", 0), old.get("retry_at", 0),
        st.session_state.get("quota_not_before", 0),
    )
    st.session_state["workflow"] = fresh
...
st.caption("Automatic recovery v1.2 · Stages are paced to reduce quota errors. Allow about four minutes of waiting plus generation time. Keep this tab open; temporary cooldowns resume automatically.")
