...
def rate_limit_info(exc):
    ...
    return seconds, text, (not daily and not oversized and seconds <= 60)
    ...
    for attempt in range(2):
        try:
            ...
            if attempt == 1 or delay is None:
                if isinstance(exc, RateLimitError):
                    seconds, message, _ = rate_limit_info(exc)
                    raise RateLimitPause(message, seconds) from None
                raise StudyError(friendly_error(exc)) from None
            # Short sleeps keep the UI progress message alive during the cooldown.
            for remaining in range(math.ceil(delay), 0, -1):
                if notify and (remaining == math.ceil(delay) or remaining % 5 == 0):
                    notify(f"Waiting for Groq: retrying this stage in {remaining} seconds...")
                time.sleep(1)
